    ...
    public class ModMain
    {
        ...
        public static void ForceRegisterItem(object confManager, object newItem, int id)
        {
            try
            {
                ...
                // [修复] 改用 GetProperty 查找 allConfDic (之前用 GetField 找不到)
                    ...
                    if (dicObj != null)
                    {
                        ...
                        if (addMethod != null && containsKeyMethod != null)
                        {
                            ...
                            if (!exists)
                            {
                                ...
                                Debug.Log($"[ForceRegister] 成功注入 ID {id} 到 {type.Name}.allConfDic");
                            }
                            else
                            {
                                Debug.Log($"[ForceRegister] ID {id} 已存在，跳过。");
                            }
                            return;
                        }
                    }
                ...
                // 如果找不到 allConfDic，尝试找 _allConfDic 字段 (以此为备选)
                ...
                if (field != null)
                {
                    ...
                    // ... 重复上述 Add 逻辑，或者简化处理 ...
                    // 实际上 V7.0 报告显示 allConfDic 属性是存在的，所以上面应该能命中
                }

                Debug.LogWarning($"[ForceRegister] 警告：在 {type.Name} 中未找到字典用于注册 ID {id}");
            }
            catch (Exception ex)
            {
                Debug.LogError($"[ForceRegister] 注入异常: {ex}");
            }
        }
