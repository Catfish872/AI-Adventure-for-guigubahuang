        ...
        public static readonly List<string> HardcodedDefaultMasterStudentEventTypes = new List<string>
{
    ...
    关于奖励物品：
    - 你可以自由创造物品或气运的名称（推荐不在列表清单中的物品，发挥想象力），系统会自动识别并将其具现化。可以不局限于已知物品，也可以使用一部分自创物品和一部分列表中物品的混合奖励。
    - 奖励必须动态调整：
    ...
    最重要：你必须返回HTML标签格式结果，不能只有剧情内容，必须是HTML标签格式内容！";
        ...
        public static string GenerateRandomSystemPrompt(WorldUnitBase npc = null, string selectedEvent = null)
        {
            ...
            rewardsString += "\n奖励说明：\n" +
                "建议你根据剧情发展、NPC身份和环境氛围，发挥想象力来设计当前故事的物品、气运或装备。自行设计的奖励可以是消耗品、装备、气运等等。\n" +
                "当你觉得没有更好的创意，或上述列表的物品已经十分符合，或需要发放基础资源（如灵石）时，可以从上述列表中选择物品。最好可以通过自主想一些不在列表清单中的物品，再加上一部分列表中物品的混合形式作为奖励。\n" +
